...
async function execute() {
  try {
    ...
    try {
      ...
      console.log('\n📊 STATUS DA SESSÃO:');
      console.log(`📁 Localização: ${path.join('auth', path.basename(sessionFile))}`);
      console.log(`🔑 Arquivo de chave: ${path.join('auth', path.basename(keyFile))} ${keyExists ? '✅' : '❌'}`);
      ...
      console.log('\n❌ Nenhuma sessão ativa encontrada em:', path.join(process.cwd(), 'auth'));
