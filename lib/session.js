...
class SessionManager {
  constructor() {
    ...
    this.sessionFile = path.join(this.authDir, 'session_data.json');
    this.keyFile = path.join(this.authDir, 'session_key.bin');
  ...
  getEncryptionKey() {
    try {
      if (fs.existsSync(this.keyFile)) {
        ...
        console.log('🔑 Nova chave de criptografia gerada em:', this.authDir);
        return key;
      }
    } catch (error) {
  ...
  async saveSession(sessionData) {
    try {
      ...
      console.log('🔒 Sessão salva com criptografia em:', this.authDir);
      return true;
    } catch (error) {
  ...
  async loadSession() {
    try {
      if (!await fs.pathExists(this.sessionFile)) {
        throw new Error('Arquivo de sessão não encontrado em: ' + this.authDir);
      }
      
      const encryptedData = await fs.readJson(this.sessionFile);
      const sessionData = this.decryptData(encryptedData);
      
      console.log('🔓 Sessão carregada e descriptografada de:', this.authDir);
      return sessionData;
    } catch (error) {
  ...
  async clearSession() {
    try {
      if (await fs.pathExists(this.sessionFile)) {
        ...
        console.log('🗑️  Sessão removida de:', this.authDir);
      }
      
      if (await fs.pathExists(this.keyFile)) {
        await fs.remove(this.keyFile);
        console.log('🗑️  Chave de criptografia removida de:', this.authDir);
      }
  ...
  async login() {
    ...
    try {
      ...
      if (saved) {
        ...
        console.log('📁 Dados salvos em:', this.authDir);
